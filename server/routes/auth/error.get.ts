const AUTH_ERROR_I18N: Record<string, { title: string, message: string }> = {
  zh: { title: '登录失败', message: '请稍后重试，窗口即将关闭…' },
  en: { title: 'Login failed', message: 'Please try again later. Window will close shortly…' }
}
...
/**
 * OAuth 失败时弹出窗口加载此页，显示错误提示并自动关闭。
 */
export default defineEventHandler((event) => {
  const locale = getLocale(event)
  const { title, message } = (AUTH_ERROR_I18N[locale] ?? AUTH_ERROR_I18N.en)!
